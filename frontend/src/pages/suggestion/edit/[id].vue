      ...
      <v-card>
        <v-toolbar dense flat>
          ...
          <div class="toolbar mb-2 d-flex align-center justify-center flex-wrap">
            ...
            <!-- Inserir Imagem -->
            <v-btn color="success" size="small" @click="openImageDialogFullScreen = true">
              <v-icon>mdi-image</v-icon>
            </v-btn>
          </div>
          <v-spacer />
    ...
    <v-dialog v-model="openImageDialog" max-width="500">
      <v-card>
        ...
        <v-card-actions>
          ...
          <v-btn color="primary" @click="insertImage(editor)">Inserir</v-btn>
        </v-card-actions>
      </v-card>
    </v-dialog>
    <!-- Modal de Inserção de Imagem fullscreen-->
    <v-dialog v-model="openImageDialogFullScreen" max-width="500">
      <v-card>
        <v-card-title>Inserir Imagem</v-card-title>
        <v-card-text>
          <v-text-field v-model="imageUrl" label="URL da Imagem" />
        ...
        <v-card-actions>
          <v-spacer />
          <v-btn text @click="openImageDialogFullScreen = false">Cancelar</v-btn>
          <v-btn color="primary" @click="insertImage(editorFullScreen)">Inserir</v-btn>
        </v-card-actions>
      </v-card>
    </v-dialog>
  </v-container>
...
// Custom image node para carregar a imagem em tamanho reduzido
const CustomImageNode = Node.create({
  ...
  addAttributes() {
    return {
      ...
      title: { default: null },
      style: {
        default: 'width: 600px; height: auto;',
...
watch(openFullScreenEditor, (newVal) => {
  if (newVal) {
    ...
    editorFullScreen.commands.setContent(editor.getJSON())
  }
})
...
function insertImage(targetEditor: Editor) {
  ...
  targetEditor
    ...
    .insertContent({
      ...
      attrs: {
        src: imageUrl.value,
        alt: 'Imagem',
        style: 'width: 600px; height: auto; border-radius: 8px; margin: 0.5rem 0;',
      },
    })
    .run()

  openImageDialog.value = false
  openImageDialogFullScreen.value = false
  imageUrl.value = ''
}
...
function applyFontSize(size: string) {
  const target = openFullScreenEditor.value ? editorFullScreen : editor
  target.chain().focus().setMark('fontSize', { style: size }).run()
}
